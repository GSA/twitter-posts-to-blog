...
require_once(dirname(__FILE__) . '/../../../wp/wp-admin/includes/post.php');
...
function dg_add_menu_item() {
	...
	wp_enqueue_script( "twitter-posts-to-blog-js", plugins_url( 'js/twitter-posts-to-blog.js', __FILE__ ), array(
			'jquery',
			'jquery-ui-core',
			'jquery-ui-tabs'
		) );
...
function dg_tw_feedback() {
	...
	<div class="updated">
		<p>
			Thanks for using this plugin, please leave feedback in the <a
				href="http://wordpress.org/plugins/twitter-posts-to-blog/">plugin page</a>
			and if you want you can <a
				href="https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=QV5Y8ZNVWGEA8">offer me a
				beer</a>
			<br/>
		...
		if ( isset( $_POST['dg_tw_item_query'] ) && is_array( $_POST['dg_tw_item_query'] ) ) {
			foreach ( $_POST['dg_tw_item_query'] as $item_query ) {
				if ( isset( $dg_tw_queryes[ urlencode( $item_query['value'] ) ] ) ) {
					...
					$dg_temp_array[ urlencode( $item_query['value'] ) ] = array(
						"value"    => $item_query['value'],
						"tag"      => $item_query['tag'],
						"last_id"  => 0,
						"firts_id" => 0
					);
...
function filter_text( $tweet, $format = "", $content = "", $limit = -1, $remove_url = false ) {
	...
	$result = str_replace( '%avatar_url%', $tweet->user->profile_image_url_https, $result );
